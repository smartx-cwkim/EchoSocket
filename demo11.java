...
class Data {
    ...
    private String container_user_name;
    private String cmd;
    private String ip;
    private String port;
    private String setcpus;
...
//    }
    ...
    public void getData() {
        ...
        this.container_user_name = scan.nextLine();
...
class Container {
    ...
    public Container (DockerClient dockerClient) {
        this.dockerClient = dockerClient;
    }
    ...
    public void createContainer(Data data, int i) {
        ...
        containerid = dockerClient.createContainerCmd(data.getImage().getId())
                .withCmd(data.getCommand(), data.getIP(), data.getPort())
                .withName(containername)
    ...
    public void listContainer() {
        List<com.github.dockerjava.api.model.Container> containers2 = dockerClient.listContainersCmd().exec();

        for(com.github.dockerjava.api.model.Container eachcontainer : containers2) {

            System.out.println(eachcontainer.getId() + " " + eachcontainer.getStatus());
            System.out.println(eachcontainer.getCommand());

            if(containerid.equals(eachcontainer.getId())) {
                System.out.println(containername);
            } else {
                System.out.println(eachcontainer.getNames().toString());
            }
        }
    }

    public void watchContainer(CountDownLatch countDownLatch, Data data) {
        StatsCallback statsCallback = dockerClient.statsCmd(containerid)
                .exec(new StatsCallback(countDownLatch, containername, data.getCount(), data.getMemory()));

        try {
            countDownLatch.await(data.getCount() * 2000, TimeUnit.MILLISECONDS);
            statsCallback.close();
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (IOException e) {
        ...
        System.out.println("Container " + containername + " stats" + status);
    }
    ...
    public void startContainer() {
        ...
        System.out.println("Container " + containername + " start " + status);
    ...
    public void killContainer() {
        ...
        System.out.println("Container " + containername + " stop " + status);
...
class Orchestrator {
    public void orchestrator (DockerClient dockerClient) {
        Data data = null;
        Container container;
        ...
        while(true) {
            ...
            System.out.print("Input what you do: ");
            ...
            if(control.equals("input")) {
                ...
                System.out.print("Do you want to check the entered value? [N/y]");
                String check = scan.nextLine();

                if (check.contains("n") || check.contains("N")) {
                    continue;
                } else {
                    System.out.println("Entered Dockerfile path: " + data.getPath());
                    System.out.println("Entered Docker image tag: " + data.getTag());
                    System.out.println("Entered Container Count: " + data.getCount());
                    System.out.println("Entered Container Command: " + data.getCommand());
                    System.out.println("Entered IP address: " + data.getIP());
                    System.out.println("Entered Port number: " + data.getPort());
                    System.out.println("Entered allocated CPU: " + data.getCPUs());
                    System.out.println("Entered allocated Memory: " + data.getenteredMemory() + " is " + data.getMemory());
                }
                ...
                Containerz ct = new Containerz();


                // If you want to get a container name not get to list, use the inspectContainerResponse.
                InspectContainerResponse inspectContainerResponse = dockerClient.inspectContainerCmd(tempxx).exec();

                System.out.println(ct.getId() + tempxx);
                System.out.println(inspectContainerResponse.getName().replace("/",""));



            } else if(control.equals("build")) {
                data.buildImage(dockerClient);

            } else if(control.equals("create")) {
                System.out.println("Create Container based on entered value");

                int size = containerList.size();

                // Have nothing to do with the container numbers.
                for(int j=size; j < size + data.getCount(); j++) {
                    // based on setting value, now make a container.
                    container = new Container(dockerClient);

                    container.createContainer(data, j);

                    containerList.put(container.getName(), container);
                }
                ...
                System.out.print("Enter the container name to want to start: ");
                String temp = scan.nextLine();

                for (String key : containerList.keySet()) {

                    if(containerList.get(key).getStatus() == 1) {
                        System.out.println("Container " + key + " already started");
                        continue;
                    }

                    if(key.equals(temp)) {
                        containerList.get(key).startContainer();
                        break;
            ...
            } else if(control.equals("stats")) {

                if(containerList.size() == 0) {
                    System.out.println("Start container first!");
                    continue;
                }

                CountDownLatch countDownLatch = new CountDownLatch(containerList.size());

                System.out.print("Enter the accurate container name to want to check resource usage: ");
                String temp = scan.nextLine();

                if(containerList.containsKey(temp)) {
                    if(containerList.get(temp).getStatus() == 1) {
                        containerList.get(temp).watchContainer(countDownLatch, data);
                    } else {
                        System.out.println("Container " + temp + " not started");
                    }
                } else {
                    System.out.println("Entered " + temp + " Container is not exist");
                }

            } else if(control.equals("kill")) {
                System.out.print("Enter the container name to want to kill: ");
                String temp = scan.nextLine();

                for (String key : containerList.keySet()) {

                    if(containerList.get(key).getStatus() != 1) {
                        System.out.println("Container " + key + " already stopped");
                        continue;
                    }

                    if(key.equals(temp)) {
                        containerList.get(key).killContainer();
                        break;
                    } else if (key.contains(temp)) {
                        containerList.get(key).killContainer();
                    }
                ...
                System.out.print("Enter the container name that already stopped to want to remove: ");
                String temp = scan.nextLine();
                ...
                for (String key : containerList.keySet()) {
                    ...
                    if(containerList.get(key).getStatus() == 1) {
                        System.out.println("Container " + key + " not stopped. Stop it first");
                        continue;
                    }

                    if(key.equals(temp)) {
                        containerList.get(key).removeContainer();
...
public class demo5 {
    ...
    public static void main(String[] args) {
        ...
        // Initialize docker client to use Docker API
        DockerCmdExecFactory dockerCmdExecFactory = new JerseyDockerCmdExecFactory()
                .withReadTimeout(1000)
                .withConnectTimeout(1000)
                .withMaxTotalConnections(100)
...
//                    .build();
        ...
        Orchestrator orchestration = new Orchestrator();

        orchestration.orchestrator(dockerClient);
...
//        }
        ...
        String afCPU_system = stats.getCpuStats().get("system_cpu_usage").toString();
        String afMemory_usage = stats.getMemoryStats().get("usage").toString();
        String network = stats.getNetworks().get("eth0").toString();
        ...
        if (stats != null) {
            ...
            System.out.println(contid + " cpu : " + cpuPercent + "%");
            System.out.println(contid + " memory : " + memoryPercent + "%");
            System.out.println(contid + " network : " + network);
        }
...
class Containerz extends com.github.dockerjava.api.model.Container{

    private int status;
    private String containerName;

    public Map<String, Containerz> bootStrap(DockerClient dockerClient, Map<String, Containerz> map) {
        List<com.github.dockerjava.api.model.Container> containers2 = dockerClient.listContainersCmd().withShowAll(true).exec();

        for(com.github.dockerjava.api.model.Container container : containers2) {

            // If you want to get a container name not get to list, use the inspectContainerResponse.
            InspectContainerResponse inspectContainerResponse = dockerClient.inspectContainerCmd(container.getId()).exec();

            containerName = inspectContainerResponse.getName().replace("/","");
