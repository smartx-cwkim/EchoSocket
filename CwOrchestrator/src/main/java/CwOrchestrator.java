...
public class CwOrchestrator {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(configuration.getCommand());

        Service checkService = orch.getAllServices().get("xxx");

        checkService.startService();
        orch.getAllServices().get("xxx").pauseService();
...
class Orchestrator {
    ...
    private Orchestrator(Configuration configuration) {
        containerMap = new HashMap<String, Service>();
        Watcher watcher;
...
//                .build();
        ...
        for(Container container : containerList) {
            ...
            Service serviceTemp = new Service(dockerClient);

            serviceTemp.setId(container.getId());

            System.out.println(inspectContainerResponse.getName().replace("/","") + inspectContainerResponse.getId() + inspectContainerResponse.getState());

            containerMap.put(inspectContainerResponse.getName(), serviceTemp);
        }
...
class Service extends Container implements Runnable{
    ...
    public Service(DockerClient dockerClient) {
    ...
    public void run() {
        try {
            ...
            while(state.contains("Running")) {
                ...
                serviceStateListener.onStateChange(containerName, state);
            }
    ...
    void buildImage(DockerClient dockerClient) {
        ...
        while(true) {
            ...
            if (check.contains("n") || check.contains("N")) {
                ...
                System.out.println("Please input follow n or Y");
            }
        }
    }
}
...
interface ServiceStateListener {
    void onStateChange(String containerName, String state);
    void onErrorDetect(String containerName, String state);
}

class Watcher{

    public Watcher(final DockerClient dockerClient, final Map<String, Service> map) {
        ...
        executor.scheduleAtFixedRate(new Runnable() {
            public void run() {
                try {
                    InspectContainerResponse icr = dockerClient.inspectContainerCmd(map.)
                } catch (Exception e) {
                    e.printStackTrace();
                    executor.shutdown();
                }
    ...
    public void onStateChange(String containerName, String state) {
        InspectContainerResponse inspectContainerResponse = dockerClient.inspectContainerCmd(containerData.get(containerName).getId()).exec();

        if(inspectContainerResponse.getState().equals(state)) {
    ...
    public void onErrorDetect(String containerName, String state) {
