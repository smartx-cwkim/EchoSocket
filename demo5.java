...
public class hello {
    ...
    public static void main(String[] args) {
        ...
        dockerfile df = null;
        int cont_number= 0;
        int CONTAINER_COUNT = 0, Memory = 0;
...
//                }
                ...
                StatsCallback statsCallback = dockerClient.statsCmd(startcont_map.get(name))
                        .exec(new StatsCallback(countDownLatch, name, startcont_map.size(), Memory));

                System.out.println(name + "fin????");

                try {
                    // start next line, when go by map.size() * 3000 MILLISECONDS.
                    countDownLatch.await(startcont_map.size() * 5000, TimeUnit.MILLISECONDS);
