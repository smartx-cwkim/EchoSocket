...
public class hello {
    ...
    public static void main(String[] args) {
        Map<String, String> map = new HashMap<String, String>();
        dockerfile df = null;
        int cont_number= 0, start_counter = 0;
        int CONTAINER_NUMBER = 0, CONTAINER_COUNT = 0, Memory = 0;
        Scanner scan = new Scanner(System.in);
...
//                    .build();
        ...
        while(true) {
            ...
            if (tp == 1) {
                ...
                // Input the value to want users
                Scanner scan3 = new Scanner(System.in);
                CONTAINER_COUNT = scan3.nextInt();
                String Clientcode = scan3.next();
                String IP = scan3.next();
                String Port = scan3.next();
                String CPU = scan3.next();
                Memory = scan3.nextInt();

                // make a container based on user's input
                settingContainer setcont = new settingContainer(dockerClient, df, map, cont_number);
                map = setcont.setContainer(CONTAINER_COUNT, Clientcode, IP, Port, CPU, Memory);
                ...
                if(name.equals("all") && start_counter == 0) {
                    stc.startMulticontainer();
                    start_counter++;
                    continue;
                } else if(name.equals("all") && start_counter != 0) {
                    stc.startMulticontainer_test(cont_number, CONTAINER_NUMBER);
                }

                if (map.containsKey(name)) {
                    stc.startSinglecontainer(name);
                }
            } else if (tp == 5) {
                CountDownLatch countDownLatch = new CountDownLatch(CONTAINER_COUNT);
...
//                }
                ...
                StatsCallback statsCallback = dockerClient.statsCmd(map.get(name))
                        .exec(new StatsCallback(countDownLatch, name, map.size(), Memory));

                System.out.println(map.get(name) + "fin????");

                try {
                    // start next line, when go by map.size() * 3000 MILLISECONDS.
                    countDownLatch.await(map.size() * 5000, TimeUnit.MILLISECONDS);
...
class startContainer {
    ...
    public startContainer(DockerClient dockerClient, Map<String, String> MAP) {
        this.map = MAP;
        this.dockerClient = dockerClient;
    }
    ...
    public void startMulticontainer_test(int number, int number2) {
...
class killContainer {
    ...
    public killContainer(DockerClient dockerClient, Map<String, String> MAP) {
        this.dockerClient = dockerClient;
        this.map = MAP;
    }
    ...
    public Map<String, String> removing(String name) {
        try {
            ...
            if(map.containsKey(name)) {
                dockerClient.removeContainerCmd(map.get(name)).exec();
                map.remove(name);
                System.out.println("Remove complete " + map.get(name));
            } else {
                System.out.println("Not removing, check the container name...");
            }
        } catch (InterruptedException e) {
...
class StatsCallback extends ResultCallbackTemplate<StatsCallback, Statistics> {
    ...
    // COUNT means map.size, not CONTAINERS_COUNT because of case of single container
    public StatsCallback(CountDownLatch countDownLatch, String CONTID, int COUNT, int Memory) {
        this.countDownLatch = countDownLatch;
        this.contid = CONTID;
        this.count = COUNT;
